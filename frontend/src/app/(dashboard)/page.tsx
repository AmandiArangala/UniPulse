...
import {
  ...
  CreditsKpiCard,
  AcademicHealthCard,
} from '@/components/student/AcademicKpiCard';
import {
  studentService,
  mockGpaSummary,
  mockTrajectory,
...
export default function DashboardOverviewPage() {
  ...
  // Student portal live data state
  ...
  const [attendanceAnalytics, setAttendanceAnalytics] = useState<OverallAttendanceAnalytics | null>(null);
  const [enrolledModules, setEnrolledModules] = useState<ModuleGradeSummary[]>([]);
    ...
    if (role === 'STUDENT') {
      const loadStudentOverview = async () => {
        ...
        try {
          ...
          const [gpa, traj, att, mods] = await Promise.all([
          ...
          setGpaSummary(gpa);
          setTrajectory(traj);
          setAttendanceAnalytics(att);
          setEnrolledModules(mods);
        } catch {
          // Fallbacks already embedded in studentService
        } finally {
      ...
      {role === 'STUDENT' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <CgpaKpiCard
              cgpa={gpaSummary.cgpa || 3.24}
              trend={0.12}
              target={`${gpaSummary.academicDegreeClass === 'FIRST_CLASS' ? 'First Class' : 'Second Class Upper'} Target`}
            />

            <AttendanceKpiCard
              attendancePercentage={attendanceAnalytics?.overallPercentage ?? 82}
              threshold={80}
            />

            <CreditsKpiCard
              earnedCredits={gpaSummary.totalEarnedGpaCredits || 64}
              totalCredits={120}
            />
          ...
          {/* Degree Honors Trajectory & Quick Navigation Banner */}
          <div className="p-4 rounded-xl bg-gradient-to-r from-indigo-50 via-slate-50 to-white dark:from-indigo-950/40 dark:via-slate-900/60 dark:to-slate-900 border border-indigo-200/80 dark:border-indigo-800/60 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">
                    On Track for {trajectory.targets?.[0]?.degreeClass === 'FIRST_CLASS' ? 'First Class (Honours)' : 'Second Upper (2:1)'}
                  </span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">
                  {trajectory.trajectorySummary || 'Current trajectory points towards Second Class (Upper Division) with First Class within reach.'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2.5 w-full md:w-auto">
              <Link
                ...
                className="flex-1 md:flex-initial px-3.5 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center space-x-1.5 shadow-xs transition-colors"
              >
              ...
              <Link
                ...
                className="flex-1 md:flex-initial px-3.5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg flex items-center justify-center space-x-1.5 shadow-xs transition-colors"
              >
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center">
                  <Activity className="w-4 h-4 mr-2 text-indigo-600" /> Enrolled Modules (Active Semester)
                </h3>
                <Link
                  href="/student/modules"
                  className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center"
                >
                  <span>View All Breakdowns</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {(enrolledModules.length > 0
                  ? enrolledModules.slice(0, 3)
                  : mockGpaSummary.semesterReports[mockGpaSummary.semesterReports.length - 1]?.modules.slice(0, 3) || []
                ).map((mod) => (
                  <Link
                    key={mod.moduleId || mod.moduleCode}
                    href={`/student/modules?highlight=${mod.moduleCode}`}
                    className="unipulse-card p-4 space-y-3 block group hover:border-indigo-400 transition-all"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <h4 className="font-bold text-sm text-slate-900 dark:text-white leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                          {mod.moduleTitle}
                        </h4>
                      </div>
                      <span className="text-xs font-black text-amber-600 bg-amber-50 dark:bg-amber-950/60 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-800">
                        {mod.letterGrade || 'B+'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-slate-800 dark:text-slate-200">
                          {mod.finalGrade ? `${mod.finalGrade}%` : '72.4%'}
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                        <div
                          className="bg-indigo-600 h-2 rounded-full transition-all duration-500"
                          style={{ width: `${mod.finalGrade || 72.4}%` }}
                        />
                      </div>
                    </div>

                    <div className="pt-1 flex items-center justify-between text-xs">
                      <span className="text-slate-500 dark:text-slate-400">Weight Split:</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-300">
                        {mod.caWeightPercentage}% CA / {mod.weWeightPercentage}% WE
                      </span>
                    </div>

                    <div className="p-2 rounded bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/60 text-[11px] text-indigo-700 dark:text-indigo-300 font-medium flex items-center justify-between">
                      <span>{mod.creditHours} Credits • {mod.lecturerName || 'Faculty Lecturer'}</span>
